...
// Export a credential to another device (phone / extension) over the relay.
// The desktop shows a QR pointing at the hosted receiver (export.html). The
// target generates its OWN did:key, sends it up the E2E channel, and the desktop
// re-issues the credential bound to that DID (useCredentialExport) and sends it
// back sealed. The target stores it and becomes a first-class holder. v1 binding
// = re-issue to device did:key (no key ever leaves a device); did:sns anchor
// registration is the later upgrade. See project_did_sns_anchor_authority.
import { ref, onMounted, onBeforeUnmount } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import QRCode from 'qrcode'
import nacl from 'tweetnacl'
import { useCredentialExport } from '../composables/useCredentialExport'
import type { VaultCredential } from '../../shared/vault-api'

const RELAY_WSS = 'wss://attestto-present-relay.fly.dev'
const EXPORT_PAGE = 'https://mobile.attestto.com/export.html'

const route = useRoute()
const router = useRouter()
const { reissueTo } = useCredentialExport()

type Step = 'starting' | 'waiting' | 'connected' | 'issuing' | 'done' | 'error'
...
async function handleMessage(raw: string) {
  ...
  if (msg.type === 'peer-joined') {
  ...
  } else if (msg.type === 'key-exchange' && !sharedKey && typeof msg.publicKey === 'string') {
    ...
    if (inner.type === 'request-export' && typeof inner.holderDid === 'string') {
      if (exported) return
      exported = true
      step.value = 'issuing'
      const cred = credential.value
      if (!cred) { errorMsg.value = 'No hay credencial para exportar'; step.value = 'error'; return }
      const vc = await reissueTo(inner.holderDid, cred)
      if (!vc) { errorMsg.value = 'No se pudo re-emitir la credencial'; step.value = 'error'; return }
      sendEncrypted({ type: 'credential', vc })
      step.value = 'done'
...
onMounted(async () => {
  try {
    ...
    const url = `${EXPORT_PAGE}?relay=${encodeURIComponent(RELAY_WSS)}&sid=${sid}&pk=${pkHex}`
    ...
    errorMsg.value = e instanceof Error ? e.message : 'Error al iniciar la exportación'
    step.value = 'error'
  }
})
...
<template>
  <q-page padding>
    <div class="row items-center q-mb-lg">
      ...
      <div class="text-h5 q-ml-sm">Exportar credencial</div>
    </div>

    <div class="column items-center q-gutter-md" style="max-width: 460px; margin: 0 auto;">
      <p class="text-grey text-center">
        Escaneá este código con tu teléfono para copiar la credencial a tu billetera móvil.
      </p>

      <div v-if="step === 'starting'" class="column items-center q-gutter-sm q-pa-lg">
        <q-spinner size="40px" color="primary" />
        <span class="text-grey">Preparando exportación…</span>
      </div>

      <div v-else-if="step === 'error'" class="text-negative text-center q-pa-lg">{{ errorMsg }}</div>

      <div v-else-if="step === 'done'" class="column items-center q-gutter-sm q-pa-lg">
        <q-icon name="check_circle" color="positive" size="56px" />
        <div class="text-h6">Credencial exportada</div>
        <p class="text-grey text-center">
          Tu dispositivo recibió una copia re-emitida a su propia llave.
        </p>
      ...
      <template v-else>
        ...
        <q-banner
          v-if="step === 'connected' || step === 'issuing'"
          dense
          class="bg-primary text-white rounded-borders"
        >
          <template #avatar><q-spinner size="20px" /></template>
          {{ step === 'connected' ? 'Dispositivo conectado…' : 'Re-emitiendo credencial…' }}
        </q-banner>
        <div v-else class="text-caption text-grey text-center">Esperando que el dispositivo escanee…</div>
        ...
        <div class="info-banner q-mt-sm">
          ...
          <span class="att-text-muted" style="font-size: var(--att-text-xs);">
            La credencial se re-emite a la llave propia del dispositivo y viaja cifrada de
            extremo a extremo. Ninguna llave privada sale de ningún dispositivo.
          </span>
        </div>
      </template>
    </div>
